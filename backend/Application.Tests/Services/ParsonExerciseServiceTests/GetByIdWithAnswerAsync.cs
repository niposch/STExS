...
public class GetByIdWithAnswerAsync : Infrastructure
{
    [Fact]
    public async Task ShouldThrowWhenNotFound()
    {
        ...
        // Act
        Func<Task> action = async () => await this.Service.GetByIdWithAnswerAsync(id, Guid.NewGuid());
    ...
    public async Task ShouldReturnCorrectly()
    {
        ...
        // Act
        var result = await this.Service.GetByIdWithAnswerAsync(parsonExercise.Id, userId);
