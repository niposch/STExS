...
export class CreateEditParsonComponent implements OnInit {
  ...
  async updateExercise() {
    this.isLoading = true;
    await lastValueFrom(
      this.parsonPuzzleService.apiParsonPuzzleUpdatePost({
        body: {
          ...this.exercise,
        },
      })
      ...
      .then(() => {
        this.snackBar.open('Successfully updated Exercise!', 'Dismiss', {
          duration: 2000,
        });
        this.isLoading = false;
        this.goBack();
      ...
      .catch((err) => {
        this.snackBar.open('Could not update Exercise!', 'Dismiss', {
          duration: 5000,
        });
        throw err;
      });
  ...
  async createExercise() {
    this.isLoading = true;
    await lastValueFrom(
      this.parsonPuzzleService.apiParsonPuzzleCreatePost$Json({
        body: {
      ...
      .catch((err) => {
        this.snackBar.open('Could not create Exercise!', 'Dismiss', {
          duration: 5000,
        });
        throw err;
      })
      .then(() => {
        this.snackBar.open('Successfully created Exercise!', 'Dismiss', {
          duration: 2000,
        });
        this.isLoading = false;
        this.goBack();
  ...
  public async goBack() {
    ...
    await this.router.navigate(['/module/administrate/chapter'], {
      queryParams: { chapterId: this.chapterId ?? this.exercise?.chapterId },
    });
  }

  private async loadExercise(
    exerciseId: string
  ): Promise<ParsonExerciseDetailItem> {
    return await lastValueFrom(
      this.parsonPuzzleService
        .apiParsonPuzzleWithAnswersGet$Json({
  ...
  drop($event: CdkDragDrop<any[], any>) {
    ...
    if (this.exercise?.lines != null) {
      this.exercise.lines = [...this.exercise.lines];
      this.changeDetectorRef.detectChanges();
    }
  }
}
